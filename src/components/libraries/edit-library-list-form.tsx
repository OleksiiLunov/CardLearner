...
function SubmitButton() {
  ...
  return (
    <Button type="submit" size="sm" disabled={pending}>
      {pending ? t("libraries.saveListPending") : t("libraries.saveChanges")}
    </Button>
  );
}

export function EditLibraryListForm({ action, initialValues }: EditLibraryListFormProps) {
  const { t } = useTranslation();
    ...
    <section className="rounded-[2rem] border border-border bg-card/80 p-5 shadow-sm backdrop-blur">
      <div className="space-y-1.5">
        <h2 className="text-lg font-semibold tracking-tight text-foreground">
          {t("libraries.editList")}
        </h2>
        <p className="max-w-xl text-sm leading-6 text-muted-foreground">
          {t("libraries.editListDescription")}
        </p>
      </div>

      <form key={formKey} action={formAction} className="mt-5 space-y-5">
        <div className="space-y-2.5">
          <Label htmlFor="edit-library-list-title">{t("libraries.listTitleLabel")}</Label>
          <Input
            id="edit-library-list-title"
            name="title"
            required
        ...
        <div className="space-y-2.5">
          <Label htmlFor="edit-library-list-description">{t("libraries.listDescriptionLabel")}</Label>
          <Textarea
            id="edit-library-list-description"
            name="description"
            rows={3}
        ...
        <div className="space-y-2.5">
          <div className="space-y-1.5">
            <Label htmlFor="edit-library-list-content">{t("libraries.listContentLabel")}</Label>
