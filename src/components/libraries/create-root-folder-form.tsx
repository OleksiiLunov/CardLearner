...
type CreateRootFolderFormProps = {
  action: (
...
function SubmitButton() {
  ...
  return (
    <Button type="submit" size="sm" disabled={pending}>
      {pending ? t("libraries.createFolderPending") : t("libraries.createFolder")}
    </Button>
  );
}

export function CreateRootFolderForm({ action }: CreateRootFolderFormProps) {
  const { t } = useTranslation();
  const [state, formAction] = useActionState(action, initialState);
  const values = state.values ?? { title: "" };
    ...
    <section className="rounded-[2rem] border border-border bg-card/80 p-5 shadow-sm backdrop-blur">
      <div className="space-y-1.5">
        ...
        <p className="max-w-xl text-sm leading-6 text-muted-foreground">
          {t("libraries.createFolderDescription")}
        </p>
      </div>

      <form key={formKey} action={formAction} className="mt-5 space-y-5">
        <div className="space-y-2.5">
          <Label htmlFor="folder-title">{t("libraries.folderTitleLabel")}</Label>
          <Input
            id="folder-title"
            name="title"
            required
